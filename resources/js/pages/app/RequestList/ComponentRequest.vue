<template >
<div v-if="role_request == 2 || role_request == 3 || role == 1 || role == 4 || role == 5 || role == 6">
    <div class="row">
        <div class="col-lg-12  z-index-41">
            <div class="panel panel-default  ">
                <div class="panel-heading no-border clearfix">
                    <h1 class="page-title">Request List Component</h1>
                </div>
                <div class="panel-body">
                    <div class="col-md-7" >
                        <router-link v-if="role == 2 || role == 3" :to="{ name: 'form_component', params: { type : '0' } }" class="btn btn-primary"> Create Request</router-link>
                        <router-link v-if="role == 2 || role == 3" :to="{ path: 'form_component/change' }" class="btn btn-success"> Change Request</router-link>
                        <router-link v-if="role == 2 || role == 3" :to="{ path: 'form_component/delete' }" class="btn btn-danger "> Delete </router-link>
                        <el-popover
                            placement="top-start"
                            width="400"
                            trigger="hover">
                            <el-form label-width="150px" label-position="right" size="mini">
                              ...
                              <el-button size="small" plain type="primary" icon="el-icon-refresh" @click="reset()">Reset</el-button>
                            </el-form>
                    ...
                    <el-table  @sort-change="sortChange" :default-sort="{prop: sort, order: order}" :data="tableData.data" style="width: 100%"
                        ...
                        <el-table-column sortable prop="request_number" label="No Request" width="130"></el-table-column>
                        <el-table-column sortable label="Request type" width="150">
                            <template slot-scope="scope">
                                {{ scope.row.request_type }} ({{ scope.row.step_request_type }})
                            </template>
                        </el-table-column>
                        <el-table-column sortable label="Part Number" width="200">
                            <template slot-scope="scope">
                                <div v-if="scope.row.part_number != null">
                                    {{ scope.row.part_number.split('|').join(', ') }}
                                </div>
                            </template>
                        </el-table-column>
                        <el-table-column sortable prop="ata_chapter" label="ATA" width="120"></el-table-column>
                        <el-table-column sortable prop="component_type" label="User Type" width="120"></el-table-column>
                        <el-table-column sortable prop="aircraft_type" label="Aircraft Type" width="200"></el-table-column>
                        <el-table-column sortable prop="created_at" label="Date" width="180"></el-table-column>
                          ...
                          <template slot-scope="scope">
                              <el-dropdown>
                                  ...
                                  <el-dropdown-menu slot="dropdown">
                                    ...
                                    <router-link :to="{ name: 'form_component_data', params: { type : scope.row.id, component_type : scope.row.component_type } }"><el-dropdown-item v-if="(scope.row.status == 0 || scope.row.status == 6 ) && scope.row.user_id == auth_id"><i class="fa fa-edit"></i> Edit Request</el-dropdown-item></router-link>
    ...
    <el-dialog title="Note Resubmit " :visible.sync="resubmitDialog" >
        <resubmit v-if="resubmitDialog == true" :id="id" type="component"/>
    </el-dialog>
</div>
</template>
  ...
  export default {
        ...
        filteredList() {
            return this.data.map(d => {
                    ...
                    .filter(data => !this.search ||
                                ...
                                data.component_request.ata_chapter != null && data.component_request.ata_chapter.toLowerCase().includes(this.search.toLowerCase()) ||
                                data.component_request.component_type != null && data.component_request.component_type.toLowerCase().includes(this.search.toLowerCase()) ||
                                data.component_request.part_number != null && data.component_request.part_number.toLowerCase().includes(this.search.toLowerCase()) ||
                                ...
                                data.component_request.aircraft_type != null && data.component_request.aircraft_type.toLowerCase().includes(this.search.toLowerCase())
                             )
        }
    },
    created() {
    ...
    data() {
        return {
            ...
            data: [] , url : BASE_URL + 'submit_component' , dialogVisible: false ,
    ...
    methods: {
      ...
      detail(data){
        if(this.role == 4 && data.status == 1){
         ...
         window.open(this.url+"/detail/"+data.id+' - '+ data.request_number + ' Component Request', '_blank')
      },
      deleted(data){
      ...
      histories(data){
          axios.get(this.url+'/'+data.id).then((res) => {
              ...
              this.dataHistory = res.data.request_history
          })
          .catch((err) => {
              console.log(err);
          })
      ...
      getData(){
        let params = {
          ...
          // ata_chapter : this.input.ata_chapter,
          // aircraft_type : this.input.aircraft_type,
          sort: this.sort,
          order: this.order
        }
