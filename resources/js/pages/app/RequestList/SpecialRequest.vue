<template >
<div class="" v-if="role_request == 4 || role == 1 || role == 4 || role == 5 || role == 6 ">
    <div class="row">
        <div class="col-lg-12  z-index-41">
            <div class="panel panel-default  ">
                <div class="panel-heading no-border clearfix">
                    <h1 class="page-title">Request List Special</h1>
                </div>
                <div class="panel-body">
                    <div class="col-md-7" >
                        <router-link v-if="role == 2 || role == 3" :to="{ path: 'form_special/Special Capability Request/add' }" class="btn btn-primary"> Create Request</router-link>
                        <router-link v-if="role == 2 || role == 3" :to="{ path: 'form_special/Special Capability Request/change' }" class="btn btn-success"> Change Request</router-link>
                        <router-link v-if="role == 2 || role == 3" :to="{ path: 'form_special/Special Capability Request/delete' }" class="btn btn-danger "> Delete </router-link>
                        <el-popover
                            placement="top-start"
                            width="400"
                            trigger="hover">
                            <el-form label-width="150px" label-position="right" size="mini">
                              ...
                              <el-form-item label="NDT Method">
                                <el-input size="small" placeholder="NDT Method" v-model="input.part_number"></el-input>
                              </el-form-item>
                    ...
                    <el-table @sort-change="sortChange" :default-sort="{prop: sort, order: order}" :data="tableData.data" style="width: 100%"
                        ...
                        <el-table-column sortable prop="part_number" label="NDT Methods" width="200"></el-table-column>
                        <el-table-column sortable prop="ata_chapter"  label="Ata Chapter" width="200"></el-table-column>
                              ...
                              <el-dropdown>
                                  ...
                                  <el-dropdown-menu slot="dropdown">
                                    <a  @click="detail(scope.row)"><el-dropdown-item ><i class="fa fa-eye"></i> Detail</el-dropdown-item></a>
                                     <router-link :to="{ name: 'form_special', params: { type : scope.row.id } }"><el-dropdown-item v-if="(scope.row.status == 0 || scope.row.status == 6 ) && scope.row.user_id == auth_id"><i class="fa fa-edit"></i> Edit Request</el-dropdown-item></router-link>
    ...
    <el-dialog title="Note Resubmit " :visible.sync="resubmitDialog" >
        <resubmit v-if="resubmitDialog == true" :id="id" type="special"/>
   </el-dialog>
</div>
</template>
  ...
  export default {
        ...
        filteredList() {
            return this.data.map(d => {
                    ...
                    .filter(data => !this.search ||
                                    ...
                                    data.special_request.part_number.toLowerCase().includes(this.search.toLowerCase()) ||
                                    data.special_request.ata_chapter.toLowerCase().includes(this.search.toLowerCase()) ||
                                    data.special_request.aircraft_type.toLowerCase().includes(this.search.toLowerCase())
                                                    )
        }
    },
    created() {
    ...
    data() {
        return {
            ...
            data: [] , url : BASE_URL + 'submit_special' , dialogVisible: false ,
    ...
    methods: {
        ...
        detail(data){
          if(this.role == 4 && data.status == 1){
           ...
           window.open(this.url+"/detail/"+data.id+' - Detail Special Request', '_blank')
        },
      deleted(data){
      ...
      viewDetail(data){
          this.datamain = data.special_request
          console.log(data);
