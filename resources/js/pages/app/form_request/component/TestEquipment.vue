...
<div class="form-horizontal">
    ...
    <div class="form-group">
        <label class="label-control col-lg-3">Equivalent (* If Available No)</label>
        <div class="col-lg-9">
            <el-upload
              class="upload-demo"
    ...
    <div class="form-group">
        <div class="col-sm-offset-2 col-sm-9">
              <el-button @click="id_test_equipment == null ? save() : update()" round icon="fa fa-plus" type="success"> {{ id_test_equipment == null ? 'Add' : 'Update' }} </el-button>
        </div>
    </div>
</div>
</template>
...
<script>
export default {
    props : ['id','component_type','id_test_equipment', 'index_test_equipment'],
    data() {
        return {
            url : BASE_URL + 'submit_component_test_equipment' ,  attach : {} ,fileList : [] , err : [] ,
            input : {},
    ...
    methods : {
        getData(){
          if(this.id_test_equipment){
            this.axios(this.url+'/'+this.id_test_equipment).then((res) => {
              this.input = res.data
            })
            .catch((err) => {
              console.log(err);
        ...
        update(){
            axios.put(this.url+'/'+this.id_test_equipment, this.input).then((res) => {
                if(res.data.status == 1){

                    this.$message({  type: 'success', message: 'Update '+this.input.name+' Success'  });
                    this.input = {}
                    this.fileList = []
                    res.data.data.index = this.index_test_equipment
                    this.$emit('addTestEquipment', res.data.data)
                }else{
